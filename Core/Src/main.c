...
const osThreadAttr_t task_heater_coo_attributes = {
  .name = "task_heater_coo",
  .stack_size = 384 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
/* USER CODE BEGIN PV */
...
heater_cooler_queueHandle = osMessageQueueNew(HEATER_COOLER_QUEUE_LEN, sizeof(HeaterCoolerCommand_t), NULL);
...
if (can_rx_queueHandle == NULL ||
  ...
  task_heater_cooHandle = osThreadNew(start_task_heater_cooler_ctrl, NULL, &task_heater_coo_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, heater_1_Pin|heater_2_Pin|STB_Pin, GPIO_PIN_RESET);
