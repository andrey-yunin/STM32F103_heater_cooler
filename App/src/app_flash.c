...
static const osMutexAttr_t configMutex_attr = {"configMutex", osMutexRecursive | osMutexPrioInherit,
                                               NULL, 0U};
...
void AppConfig_FactoryReset(void) {
    ...
    (void)HAL_FLASHEx_Erase(&erase_config, &page_error);
...
bool AppConfig_Commit(void) {
    ...
    if (status == HAL_OK) {
        ...
        for (uint32_t offset = 0U; offset < sizeof(g_app_config); offset += sizeof(uint32_t)) {
            status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, flash_address, *config_words);
