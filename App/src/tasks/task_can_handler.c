...
static void CAN_ConfigureFilterBank(uint8_t bank,
									uint8_t destination)
{
    ...
    filter_id = CAN_BUILD_ID(
    		CAN_PRIORITY_HIGH,
			CAN_MSG_TYPE_COMMAND,
			destination,
			0U);

    filter_mask = CAN_FILTER_MSGTYPE_DST_MASK |
    			CAN_FILTER_IDE;

    filter_config.FilterBank = bank;
    filter_config.FilterMode = CAN_FILTERMODE_IDMASK;
    filter_config.FilterScale = CAN_FILTERSCALE_32BIT;
    filter_config.FilterIdHigh = (uint16_t)((filter_id << 3) >> 16);
    filter_config.FilterIdLow = (uint16_t)((filter_id << 3) & 0xFFFFU);
...
static void CAN_QueueTxFrame(const CanTxFrame_t *tx_frame)
{
	...
	if (osMessageQueuePut(can_tx_queueHandle,
						tx_frame,
						0U,
						0U) == osOK)
		{
...
void CAN_Diagnostics_GetSnapshot(CanDiagnostics_t *out)
{
    ...
    __disable_irq();
    memcpy(out,
           (const void *)&g_can_diag,
           sizeof(CanDiagnostics_t));
...
static bool CAN_IsAcceptedCommand(const CanRxFrame_t *rx_frame)
{
    ...
    if (CAN_GET_MSG_TYPE(rx_frame->header.ExtId) !=
    		CAN_MSG_TYPE_COMMAND)
    {
    	return false;
    }
							...
							ParsedCanCommand_t *parsed_command)
{
	...
	parsed_command->cmd_code =
			(uint16_t)rx_frame->data[0] |
			((uint16_t)rx_frame->data[1] << 8);

	parsed_command->device_id = rx_frame->data[2];

	memcpy(parsed_command->data,
			&rx_frame->data[3],
...
void CAN_SendNack(uint16_t cmd_code,
				uint16_t error_code)
                ...
                uint8_t len)
{
 ...
 void CAN_SendDone(uint16_t cmd_code,
		 	 	 uint8_t target_id)
 {
	 CanTxFrame_t tx_frame;
...
void app_start_task_can_handler(void *argument)
{
    ...
    CAN_ConfigureFilterBank(1U, CAN_NODE_ID);
    ...
    for (;;)
    	...
    	{
    		while (osMessageQueueGet(can_rx_queueHandle,
                                       &rx_frame,
                                       NULL,
                                       0U) == osOK)
    			{
    	...
    	if ((flags & FLAG_CAN_TX) != 0U)
    		{
    		while (osMessageQueueGet(can_tx_queueHandle,
    								&tx_frame,
									NULL,
									0U) == osOK)
    			{
