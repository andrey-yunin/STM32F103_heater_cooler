...
// --- Публикация прогресса CAN-задачи ---
...
static void CAN_ParseCommand(const CanRxFrame_t *rx_frame,
		ParsedCanCommand_t *parsed_command) {
	memset(parsed_command, 0, sizeof(*parsed_command));
	parsed_command->cmd_code = (uint16_t) rx_frame->data[0]
			| ((uint16_t) rx_frame->data[1] << 8);

	parsed_command->device_id = rx_frame->data[2];

	memcpy(parsed_command->data, &rx_frame->data[3],
			sizeof(parsed_command->data));
...
void CAN_SendAck(uint16_t cmd_code) {
	...
	tx_frame.header.ExtId = CAN_BUILD_ID(CAN_PRIORITY_NORMAL, CAN_MSG_TYPE_ACK,
...

{
	...
	tx_frame.header.ExtId = CAN_BUILD_ID(CAN_PRIORITY_NORMAL, CAN_MSG_TYPE_NACK,
			CAN_ADDR_CONDUCTOR, (uint8_t )AppConfig_GetPerformerID());
...
void app_start_task_can_handler(void *argument) {
	...
	/*
	 * CMSIS принимает timeout в тиках.
	 * При текущих 1000 Гц значение составляет 500 тиков.
	 */
	...
	for (;;) {
		...
		// --- Ожидание события или штатное idle-пробуждение ---
		flags = osThreadFlagsWait(
		FLAG_CAN_RX | FLAG_CAN_TX,
		osFlagsWaitAny, idle_wait_ticks);

		/*
		 * Отсутствие трафика не является отказом.
		 * Возврат по timeout подтверждает, что задача исполняется.
		 */
		...
		/*
		 * Прочие ошибки ожидания не подтверждают прогресс.
		 * Сохраняем прежний переход к следующей попытке,
		 * но не публикуем heartbeat из ошибочного пути.
		 */
		...
		/*
		 * Задача дошла до обработки события.
		 * Если обработка RX/TX зависнет, новых отметок не будет.
		 */
		AppWatchdog_Heartbeat(APP_WDG_CLIENT_CAN);
		...
		if ((flags & FLAG_CAN_RX) != 0U) {
			while (osMessageQueueGet(can_rx_queueHandle, &rx_frame, NULL, 0U)
					== osOK) {
				...
				if (osMessageQueuePut(dispatcher_queueHandle, &parsed_command,
						0U, 0U) == osOK) {
					g_can_diag.rx_total++;
				} else {
		...
		if ((flags & FLAG_CAN_TX) != 0U) {
			while (osMessageQueueGet(can_tx_queueHandle, &tx_frame, NULL, 0U)
					== osOK) {
				...
				if (HAL_CAN_AddTxMessage(&hcan, &tx_frame.header, tx_frame.data,
						&tx_mailbox) == HAL_OK) {
					g_can_diag.tx_total++;
				} else {
